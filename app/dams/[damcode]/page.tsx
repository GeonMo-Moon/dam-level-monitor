...
export default async function DamDetailPage({
  ...
  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 max-w-3xl mx-auto w-full">
      <Link href="/dams" className="text-sm text-blue-600 hover:underline mb-4 inline-block">
        ← 전체 목록
      </Link>

      {/* 헤딩 — 모바일: 세로 스택, 데스크톱: 가로 */}
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-5">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">{dam.damnm}</h1>
      ...
      {/* 현재 수치 카드 — 모바일: 게이지 위, 수치 아래 / 데스크톱: 가로 */}
      <div className="flex flex-col sm:flex-row gap-3 mb-4 sm:items-stretch">
        {/* 저수율 게이지 */}
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-center sm:shrink-0">
        ...
        <div className="grid grid-cols-3 gap-3 flex-1">
          {[
            ...
            <div key={label} className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 text-center">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-500 mt-0.5">
                현재 저수량 기준 · 1인 하루 300L · 전국민 5,100만명
              </p>
            </div>
          </div>
        );
